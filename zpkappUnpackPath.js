...
var pos = file.lastIndexOf(".");
	if(file.substring(pos,file.length) == ".zpk"){
		
		const zpkfile = path.join(folder,file)
		console.log("unpack zpkfile : " + zpkfile);
		
		const filename = zpkfile.substring(0, zpkfile.lastIndexOf("."));
		try {
		...
		fs.readFile(zpkfile, (error, data) => unpackZpk(data, (item) => { fs.writeFile(filename + "/" + item.filename, item.data, err => err ? console.log(err) : ""); }));
	}
});
